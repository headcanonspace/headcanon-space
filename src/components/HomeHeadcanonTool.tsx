  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto p-4 md:p-8 font-[family-name:var(--font-geist-sans)] tracking-tight">
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-stretch">
        ...
        {/* LEFT PANEL */}
        <div className="w-full lg:w-[35%] bg-white border border-[var(--color-border)] rounded-xl p-6 md:p-8 flex flex-col gap-6 shadow-2xl">
          ...
          <div key={genKey} className="flex-1 flex flex-col gap-4 motion-safe:animate-[fadeIn_0.35s_ease-out_both]">
            {results.length > 0 ? (
              results.map((item, idx) => (
                <div key={item.id} className="bg-white border border-[var(--color-border)] rounded-lg p-5 md:p-6 flex flex-col gap-3 group transition-all duration-300" style={{ animationFillMode: "both", animation: `fadeIn 0.3s ease-out ${idx * 0.1}s forwards` }}>
                  <div className="flex justify-between items-start mb-0.5">
                    <h3 className="text-[var(--color-text)] font-semibold text-[15px] flex items-center gap-2">{item.title}</h3>
                    <button onClick={() => handleCopy(item.id, item.text)} className="text-[var(--color-ink-3)] hover:text-[var(--color-primary)] transition-colors bg-transparent p-1.5 -mr-1.5 -mt-1.5 rounded-md hover:bg-[var(--color-primary-lt)] active:scale-95" title="Copy to clipboard">
                      {copiedId === item.id ? (
