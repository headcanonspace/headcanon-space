...
import { Zap, RefreshCw, Copy, Check, Plus, Trash2, PenLine, CircleDollarSign } from "lucide-react";
import {
  generateHeadcanons,
...
const STORAGE_KEY = "headcanon.relationship.lastResult";

const EXAMPLE_PRESETS: ExamplePreset[] = [
  { title: "Harry & Hermione", characters: ["Harry", "Hermione"], fandom: "Harry Potter", type: "Friendship", tone: "Wholesome", context: "Hogwarts school days and adventures" },
  { title: "Elsa & Anna", characters: ["Elsa", "Anna"], fandom: "Frozen", type: "Family", tone: "Emotional", context: "" },
  { title: "Peter & Tony", characters: ["Peter Parker", "Tony Stark"], fandom: "Marvel", type: "Mentor-Student", tone: "Dramatic", context: "" },
];
...
export default function RelationshipHeadcanonGenerator() {
  ...
  const handleGenerate = useCallback(() => {
    ...
    setResult(generated);
    setGenKey((k) => k + 1);
  }, [canGenerate, characters, fandom, relationshipType, tone, length, context]);
        ...
        <div className="w-full lg:w-[420px] lg:shrink-0">
          <div className="rounded-2xl border border-[var(--color-border)] bg-white p-5 sm:p-7 shadow-2xl space-y-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="text-lg font-bold text-[var(--color-text)]">The Foundry</h2>
            </div>

            <div className="space-y-3">
              <label className="block text-xs uppercase tracking-wider font-semibold text-[var(--color-ink-4)]">Subjects <span className="text-[var(--color-ink-4)] font-normal normal-case">(Max 5)</span></label>
              ...
              {characters.length < 5 && (
                <button type="button" onClick={addCharacter} className="w-full flex items-center justify-center gap-1.5 rounded-xl border border-dashed border-slate-300 text-[var(--color-ink-4)] py-2.5 text-sm font-medium hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-primary)] hover:border-[var(--color-primary)] transition-colors cursor-pointer active:scale-[0.99]"><Plus className="size-3.5" strokeWidth={2.5} aria-hidden />Add Subject</button>
              )}
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs uppercase tracking-wider font-semibold text-[var(--color-ink-4)]">Universe <span className="text-[var(--color-ink-4)] font-normal normal-case">(Optional)</span></label>
              <input type="text" value={fandom} onChange={(e) => setFandom(e.target.value)} placeholder="e.g., Cyberpunk, Forgotten Realms..." className="w-full rounded-xl border border-[var(--color-border)] bg-white px-4 py-3 sm:py-2.5 text-sm text-[var(--color-text)] placeholder-[var(--color-ink-4)] outline-none transition-all focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]" />
            </div>

            <div className="h-px w-full bg-gradient-to-r from-transparent via-[var(--color-border)] to-transparent my-4"></div>

            <SelectField label="Dynamic" value={relationshipType} onChange={setRelationshipType} options={RELATIONSHIP_TYPES} />
            ...
            <div className="space-y-1.5 pt-2">
              ...
              <textarea value={context} onChange={(e) => setContext(e.target.value)} placeholder="Set the scene, AU, or specific tension..." rows={3} className="w-full rounded-xl border border-[var(--color-border)] bg-white px-4 py-3 sm:py-2.5 text-sm text-[var(--color-text)] placeholder-[var(--color-ink-4)] outline-none transition-all focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)] resize-none" />
            </div>
            ...
            <div className="rounded-2xl border border-[var(--color-border)] bg-white p-6 sm:p-10 shadow-2xl min-h-[500px] flex flex-col justify-center relative overflow-hidden">
              ...
              <div className="flex flex-col items-center text-center space-y-6 max-w-md mx-auto">
                <div className="w-20 h-20 rounded-2xl bg-[var(--color-bg)] border border-[var(--color-border)] flex items-center justify-center text-[var(--color-primary)] shadow-inner">
                  <CircleDollarSign className="size-8" strokeWidth={1.5} aria-hidden />
                </div>
                <div className="space-y-3">
                  <h3 className="text-xl font-bold text-[var(--color-text)]">Awaiting Submissions</h3>
                  <p className="text-sm text-[var(--color-ink-4)] leading-relaxed">The forge remains dormant. Enter subject parameters in the adjoining console to synthesize new relationship pathways.</p>
                </div>
              </div>
              <div className="mt-12 pt-8 border-t border-[var(--color-border)]">
                <div className="flex items-center gap-3 mb-5">
                  <div className="h-px flex-1 bg-[var(--color-bg)]" />
                  <span className="text-xs uppercase tracking-widest font-semibold text-[var(--color-ink-2)]">Known Schematics</span>
                  <div className="h-px flex-1 bg-[var(--color-bg)]" />
                </div>
